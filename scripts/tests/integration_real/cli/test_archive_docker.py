...
class TestDockerArchiveCreation:
    ...
    def _docker_run(self, template_dir: Path, output_dir: Path, cmd: list):
        subprocess.run(
            ["docker", "run", "--rm",
             "-v", f"{_to_docker_path(template_dir)}:/workspace/template:ro",
             "-v", f"{_to_docker_path(output_dir)}:/workspace/output",
             self.IMAGE] + cmd,
            check=True,
            capture_output=True,
        )
    ...
    def test_zip_archive_created(self, tmp_path):
        ...
        output_dir.mkdir()
        self._docker_run(template_dir, output_dir, [
            "7z", "a", "-tzip", "/workspace/output/test.zip", "/workspace/template/*",
        ])
        assert (output_dir / "test.zip").exists()
        assert (output_dir / "test.zip").stat().st_size > 0

    def test_zip_is_extractable(self, tmp_path):
        template_dir = tmp_path / "template"
        output_dir = tmp_path / "output"
        _create_minimal_template(template_dir, num_files=2)
        output_dir.mkdir()
        self._docker_run(template_dir, output_dir, [
            "7z", "a", "-tzip", "/workspace/output/archive.zip", "/workspace/template/*",
        ])
    ...
    def test_7z_archive_created(self, tmp_path):
        template_dir = tmp_path / "template"
        output_dir = tmp_path / "output"
        _create_minimal_template(template_dir, num_files=2)
        output_dir.mkdir()
        self._docker_run(template_dir, output_dir, [
            "7z", "a", "-t7z", "/workspace/output/test.7z", "/workspace/template/*",
        ])
        assert (output_dir / "test.7z").exists()
        assert (output_dir / "test.7z").stat().st_size > 0

    def test_tar_gz_archive_created(self, tmp_path):
        template_dir = tmp_path / "template"
        output_dir = tmp_path / "output"
        _create_minimal_template(template_dir, num_files=2)
        output_dir.mkdir()
        self._docker_run(template_dir, output_dir, [
            "tar", "-czf", "/workspace/output/test.tar.gz",
