...
class TestRunTask:
    def test_returns_true_on_success(self, tmp_path):
        runner = FakeSubprocessRunner().set_popen(["Task :app:assembleDebug\n", "BUILD SUCCESSFUL\n"], returncode=0)
        assert GradleRunner(runner, tmp_path).run_task("assembleDebug") is True

    def test_returns_false_on_failure(self, tmp_path):
        runner = FakeSubprocessRunner().set_popen(["FAILURE: Build failed\n"], returncode=1)
    ...
    def test_returns_false_on_timeout(self, tmp_path):
        ...
        fake_popen.wait = lambda timeout=None: (_ for _ in ()).throw(subprocess.TimeoutExpired("gradlew", 600))
        runner.popen
