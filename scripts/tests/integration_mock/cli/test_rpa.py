#!/usr/bin/env python3
"""Integration-real tests for RPA archive creation — pure Python, zero subprocess.
...
class TestRpaRealCreation:
    ...
    def test_all_files_in_index(self, tmp_path):
        c, tmpl, out = _make(tmp_path, num_files=3)
        c.create_rpa()
    ...
    def test_stored_bytes_match_original(self, tmp_path):
        c, tmpl, out = _make(tmp_path, num_files=2)
        c.create_rpa()
